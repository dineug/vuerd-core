...
export function split (container: View, direction: Direction, tab: Tab, tabView: View, targetView: View) {
  ...
  if (direction !== Direction.all) {
    if (targetView.parent) {
      ...
      switch (direction) {
        case Direction.top:
          if (parentView.vertical) { // children split
          ...
          } else if (parentView.horizontal) { // parent add
        ...
        case Direction.bottom:
          ...
          break
        case Direction.left:
          if (parentView.vertical) { // parent add
            const targetIndex = parentView.children.indexOf(targetView)
            parentView.children.splice(targetIndex, 0, addView(parentView, [tab]))
            resetSize(parentView)
          } else if (parentView.horizontal) { // children split
            const tabs = targetView.tabs
            targetView.tabs = []
