...
export default class TreeView extends Vue {
  ...
  private onDraggableStart() {
    ...
    ul.childNodes.forEach((child: ChildNode) =>
      targets.push(child.childNodes[1])
    );
    ...
    targets.forEach((child: ChildNode) => {
      ...
      if (el.dataset.folder === "true") {
        this.draggableObservable.push({
          subDragover: fromEvent<DragEvent>(el, "dragover")
            .pipe(throttleTime(100))
            .subscribe(this.onDragoverFolder),
          subDragleave: fromEvent<DragEvent>(el, "dragleave").subscribe(
